...
def update_web_search_settings_from_env(
    container_name: str = "open-webui", env_vars: Optional[Dict[str, str]] = None
) -> bool:
    """
...
"""
        ...
        if check_result.returncode == 0:
            if "EXISTING_ENGINE:" in check_result.stdout:
                existing_engine = check_result.stdout.split("EXISTING_ENGINE:")[
                    1
                ].strip()
        ...
        for env_key, config_path in env_to_config.items():
            ...
            if env_key in env_vars and env_vars[env_key]:
                env_value: str = env_vars[env_key]
                lowered = env_value.lower()
                converted_value: Any
                if lowered in ("true", "false"):
                    converted_value = lowered == "true"
                else:
                    try:
                        if "." not in env_value:
                        ...
                        converted_value = env_value  # Keep as string
                updates[config_path] = converted_value

        # If bypass_web_loader is True, also clear loader.engine to prevent Playwright usage
        if (
            "rag.web.search.bypass_web_loader" in updates
            and updates["rag.web.search.bypass_web_loader"] is True
        ):
...
def update_web_search_settings_on_start(container_name: str = "open-webui") -> bool:
    ...
    logger.warning(
        f"Container {container_name} did not start within {max_wait} seconds"
    )
    return False
