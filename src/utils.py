...
DEPRECATED: Use infrastructure.output and infrastructure.file_repository for new code.

This module is kept for backward compatibility only.
All new code should use:
- infrastructure.output for formatted output (print_*, Colors, ask_yes_no)
- infrastructure.file_repository for file operations (read_env_file, set_permissions, etc.)
"""
...
# Re-export read_env_file from infrastructure.file_repository
from .infrastructure.file_repository import FileRepository

# Re-export from infrastructure.output for backward compatibility
...
# Re-export _ask_yes_no as alias for backward compatibility
_ask_yes_no = ask_yes_no


def read_env_file(file_path: Union[str, Path] = ".env") -> Dict[str, str]:
    """
    Read environment variables from .env file
    DEPRECATED: Use FileRepository.read_env_file() instead
    ...
    """
    file_repo = FileRepository(Path.cwd())
    return file_repo.read_env_file(Path(file_path))


def set_file_permissions(file_path: str, mode: int = 0o600) -> None:
    """
    Set file permissions
    DEPRECATED: Use FileRepository.set_permissions() or os.chmod() directly
...
def ensure_dir(dir_path: str) -> None:
    ...
    DEPRECATED: Use Path.mkdir(parents=True, exist_ok=True) directly
...
def check_root() -> None:
    ...
    TODO: Move to infrastructure.platform_utils or separate module
    """
    if os.geteuid() == 0:
        ...
        if script_dir != app_dir:
            ...
            print(
                f"   This is not the system installation directory ({SYSTEM_APP_DIR})"
            )
            print()
            print("Options:")
            print(
                "  1. Create system user and copy files to /opt/ai-gateway (recommended for production)"
            )
            ...
            if setup_user_script.exists() and os.access(setup_user_script, os.X_OK):
                ...
                if response:
                    ...
                    try:
                        ...
                        print()
                        print_success(
                            f"System user '{SYSTEM_USERNAME}' created and files copied to {SYSTEM_APP_DIR}!"
                        )
                        print()
                        print("Next steps:")
                        print(
                            f"  1. Switch to the new user: sudo -u {SYSTEM_USERNAME} -s"
                        )
                        print(f"  2. Navigate to: cd {SYSTEM_APP_DIR}")
                        print("  3. Run setup: ./setup.sh (or ./ai-gateway setup)")
                        print()
                        sys.exit(0)
        ...
        if user_exists:
                    ...
                    try:
                        ...
                        if (app_dir / ".git").exists():
                            ...
                            if source_hash and app_hash and source_hash != app_hash:
                                needs_update = True
                                update_reason = (
                                    "source code is newer (different git commits)"
                                )
                        elif source_hash:
                            # App dir is not a git repo, check modification time
                            if (script_dir / "src" / "ports.py").exists() and (
                                app_dir / "src" / "ports.py"
                            ).exists():
                                source_mtime = (
                                    (script_dir / "src" / "ports.py").stat().st_mtime
                                )
                                app_mtime = (
                                    (app_dir / "src" / "ports.py").stat().st_mtime
                                )
                                if source_mtime > app_mtime:
                                    needs_update = True
                                    update_reason = (
                                        "source code is newer (file modification time)"
                                    )
                    except Exception:
            ...
            if needs_update:
                ...
                print()
                print(
                    f"💡 We recommend updating {SYSTEM_APP_DIR} before running setup."
                )
                print("   This will ensure you have the latest code with all features.")
                print()

                if ask_yes_no("Update files in /opt/ai-gateway first?", default=True):
                    print()
                    ...
                    if setup_user_script.exists() and setup_user_script.is_file():
                        try:
                            ...
                            if result.returncode == 0:
                                ...
                                print()
                                print_warning(
                                    f"Update completed with exit code {result.returncode}."
                                )
                                print(
                                    "   Check the output above for specific warnings or errors."
                                )
                                print()
                        except Exception as e:
            ...
            if ask_yes_no(f"Run setup as user '{SYSTEM_USERNAME}'?", default=True):
                print()
                ...
                print(
                    f"  1. Run setup manually: sudo -u {SYSTEM_USERNAME} {SYSTEM_APP_DIR}/setup.sh"
                )
        ...
        print(
            "For production deployment, we recommend creating a dedicated system user."
        )
        print()
        ...
        try:
            ...
            print(
                f"💡 We can create a system user '{SYSTEM_USERNAME}' (like www-data) for you."
            )
            ...
            if not ask_yes_no("Create system user and setup?", default=True):
                print()
                print("Setup cancelled.")
                print()
                print("Alternative options:")
            ...
            try:
                ...
                if setup_script.exists() and setup_script.is_file():
                    ...
                    print()
                    print(
                        f"Would you like to run the initial setup now as user '{SYSTEM_USERNAME}'?"
                    )
                    ...
                    if ask_yes_no("Run setup now?", default=True):
                        ...
                        try:
                            ...
                            if result.returncode == 0:
                                ...
                                print(
                                    f"  sudo -u {SYSTEM_USERNAME} {SYSTEM_APP_DIR}/start.sh"
                                )
                                print()
                                print("Or use systemd service:")
                                print(
                                    "  sudo systemctl start ai-gateway  # for regular Docker"
                                )
                                print(
                                    f"  sudo -u {SYSTEM_USERNAME} systemctl --user start ai-gateway  # for rootless Docker"
                                )
                            else:
                                print()
                    ...
                    print(f"  2. Navigate to: cd {SYSTEM_APP_DIR}")
                    print(
                        f"  3. Run setup: sudo -u {SYSTEM_USERNAME} {SYSTEM_APP_DIR}/setup.sh"
                    )
                    print()
...
def get_user() -> str:
    ...
    TODO: Move to infrastructure.platform_utils
    """
