...
from ..core.config import AppConfig, ResourceProfile, BudgetProfile, PortConfig
...
class ConfigService:
    ...
    def load_from_env(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Invalid budget profile: {budget_str}, using {BUDGET_PROFILE_TEST}")
        ...
        from ..core.constants import (
            DEFAULT_POSTGRES_PORT, DEFAULT_LITELLM_PORT, DEFAULT_WEBUI_INTERNAL_PORT,
            MIN_PORT, MAX_PORT
        )
        ...
        try:
            postgres_port = int(env_vars.get("POSTGRES_PORT", str(DEFAULT_POSTGRES_PORT)))
            ...
            raise ValidationError(f"Invalid PostgreSQL port value in .env: {env_vars.get('POSTGRES_PORT')}") from e
        
        try:
            litellm_port = int(env_vars.get("LITELLM_INTERNAL_PORT", str(DEFAULT_LITELLM_PORT)))
            ...
            raise ValidationError(f"Invalid LiteLLM port value in .env: {env_vars.get('LITELLM_INTERNAL_PORT')}") from e
        
        try:
            webui_port = int(env_vars.get("WEBUI_INTERNAL_PORT", str(DEFAULT_WEBUI_INTERNAL_PORT)))
            ...
            raise ValidationError(f"Invalid WebUI port value in .env: {env_vars.get('WEBUI_INTERNAL_PORT')}") from e
        ...
        if litellm_ext:
            try:
                ...
                raise ValidationError(f"Invalid LiteLLM external port value in .env: {litellm_ext}") from e
        else:
        ...
        if webui_ext:
            try:
                ...
                raise ValidationError(f"Invalid WebUI external port value in .env: {webui_ext}") from e
        else:
            port_config.webui_external_port = None
        
        # Nginx settings
        port_config.use_nginx = env_vars.get("USE_NGINX", "").lower() in ("yes", "true", "1")
        ...
        if nginx_http:
            try:
                ...
                raise ValidationError(f"Invalid Nginx HTTP port value in .env: {nginx_http}") from e
        else:
        ...
        if nginx_https and nginx_https.lower() != 'none':
            try:
                ...
                raise ValidationError(f"Invalid Nginx HTTPS port value in .env: {nginx_https}") from e
        else:
        ...
        if nginx_port and nginx_port.lower() != 'none':
            try:
                ...
                raise ValidationError(f"Invalid Nginx port value in .env: {nginx_port}") from e
        else:
