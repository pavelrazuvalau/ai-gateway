...
from .core.exceptions import ConfigurationError, PortError, ValidationError
...
def generate_random_high_port(
    used_ports: Optional[Dict[int, str]] = None, max_attempts: int = 100
) -> int:
    """
    ...
    for _ in range(max_attempts):
        ...
        if used_ports is None or port not in used_ports:
            ...
            if is_available:
                return port
    raise PortError(
        f"Could not find available port in range {HIGH_PORT_MIN}-{HIGH_PORT_MAX} after {max_attempts} attempts"
    )
...
def select_port_interactive(
    ...
    if not (MIN_PORT <= default_port <= MAX_PORT) and default_port not in (80, 443):
        raise ValidationError(
            f"Default port {default_port} is out of valid range ({MIN_PORT}-{MAX_PORT})"
        )
    ...
    if default_port < 1024 and is_rootless_docker() and not is_internal:
        ...
        print_info(
            "Rootless Docker cannot use privileged ports without special configuration"
        )
    ...
    if is_internal:
        print(
            f"  {Colors.BLUE}[1] Default port {default_port}{Colors.RESET} (recommended)"
        )
        print(f"  {Colors.BLUE}[2] Specify custom port{Colors.RESET} (for security)")
        print(
            f"  {Colors.BLUE}[3] Random high port{Colors.RESET} ({HIGH_PORT_MIN}-{HIGH_PORT_MAX}, enhanced security)"
        )
        print()
            ...
            if choice == "1":
                ...
                while True:
                    try:
                        port = int(
                            input(
                                "Enter port number (1024-65535, or 80/443 for nginx): "
                            )
                        )
                        if (1024 <= port <= 65535) or port in (80, 443):
                            ...
                            if port < 1024 and port not in (80, 443):
                                if is_rootless_docker():
                                    ...
                                    print_info(
                                        "Please use a port >= 1024, or select option [4] for random high port"
                                    )
                                    continue
                            ...
                            if used_ports and port in used_ports:
                                print_warning(
                                    f"Port {port} is already used for {used_ports[port]}."
                                )
                            ...
                            if default_ports and port in default_ports:
                                ...
                                print_warning(
                                    f"Port {port} is standard for {default_ports[port]}."
                                )
                                print_info("This may cause issues on startup.")
                                confirm = (
                                    input("Use this port anyway? [y/N]: ")
                                    .strip()
                                    .lower()
                                )
                                if confirm != "y":
                            ...
                            print_error(
                                "Invalid port. Enter a number from 1024 to 65535, or 80/443 for nginx"
                            )
                    except ValueError:
                ...
                try:
                    ...
                    print_success(
                        f"Generated random high port: {port} (range: {HIGH_PORT_MIN}-{HIGH_PORT_MAX})"
                    )
                    if used_ports is not None:
                        used_ports[port] = service_name
                    return port
        ...
        print(
            f"  {Colors.BLUE}[2] Port {DEFAULT_NGINX_HTTPS_PORT}{Colors.RESET} (HTTPS, SSL certificate required)"
        )
        print(f"  {Colors.BLUE}[3] Specify custom port{Colors.RESET}")
        print(
            f"  {Colors.BLUE}[4] Random high port{Colors.RESET} ({HIGH_PORT_MIN}-{HIGH_PORT_MAX}, enhanced security)"
        )
        print()
        ...
        while True:
                ...
                while True:
                    try:
                        ...
                        if (1024 <= port <= 65535) or port in (80, 443):
                            ...
                            print_info(
                                f"Or select option [4] for random high port ({HIGH_PORT_MIN}-{HIGH_PORT_MAX})"
                            )
                    except ValueError:
                        print_error("Invalid format. Enter a number")
                        print_info(
                        ...
                        retry = (
                            input("Try again? [Y/n] or type '4' for random port: ")
                            .strip()
                            .lower()
                        )
                        if retry == "4":
            ...
            if used_ports and selected_port in used_ports:
                print_warning(
                    f"Port {selected_port} is already used for {used_ports[selected_port]}."
                )
                print_info("Select another port.")
                continue
            ...
            if choice != "2" and default_ports and selected_port in default_ports:
                print_warning("Port conflict!")
                print_warning(
                    f"Port {selected_port} is standard for {default_ports[selected_port]}."
                )
                print_info("This may cause issues on startup.")
                confirm = input("Use this port anyway? [y/N]: ").strip().lower()
                if confirm != "y":
...
def configure_ports(non_interactive: Optional[bool] = None) -> Dict[str, Any]:
    """
    Configure all ports (internal and external).

    Args:
        non_interactive: If True, skip prompts and use default values. If None, check NON_INTERACTIVE env var.
    ...
    """
    from .infrastructure.output import Colors, is_non_interactive, print_header
    ...
    if is_rootless_docker():
        ...
        print_info(
            "Rootless Docker cannot use privileged ports (< 1024) without special configuration"
        )
    ...
    print(
        f"{Colors.BLUE}Internal ports{Colors.RESET} - only for container-to-container communication"
    )
    ...
    if non_interactive:
        # In non-interactive mode, use nginx by default
        use_nginx = True
        print_info("Non-interactive mode: using nginx (reverse proxy)")
    else:
        ...
        print()
        use_nginx_input = input("Use nginx (reverse proxy)? [Y/n]: ").strip().lower()
        use_nginx: bool = use_nginx_input != "n"  # Default is 'y'
    ...
    if use_nginx:
        ...
        print_info(
            "Note: SSL/HTTPS configuration should be done via your own nginx container"
        )
    ...
    print(
        f"{Colors.BLUE}[2] Configure ports manually{Colors.RESET} (for customization)"
    )
    print(
        f"{Colors.BLUE}[3] Use random high ports{Colors.RESET} ({HIGH_PORT_MIN}-{HIGH_PORT_MAX}, enhanced security)"
    )
    print()
    ...
    if non_interactive:
        # In non-interactive mode, use default ports
        choice = "1"
        print_info("Non-interactive mode: using default ports")
    else:
        choice = None
    ...
    while True:
        ...
        if choice == "1":
            ...
            def check_default_port(port: int, service_name: str) -> None:
                # Closure uses default_conflicts from outer scope - this is intentional
                owner = host_ports_in_use.get(port)
                if owner:
                    default_conflicts.append((service_name, port, owner))  # noqa: B023
            ...
            if default_conflicts:
                ...
                for service_name, port, owner in default_conflicts:
                    print_warning(
                        f"  • {service_name} cannot use port {port}, it's already in use: {owner}"
                    )
            ...
            if use_nginx:
                ...
                print_success(
                    f"Using random high port for nginx: {nginx_port} (rootless Docker standard)"
                )
            ...
            print_info(
                f"Port range: {HIGH_PORT_MIN}-{HIGH_PORT_MAX} (enhanced security)"
            )
            print()
            ...
            if use_nginx:
                ...
                litellm_external_port = generate_random_high_port(
                    used_ports=host_ports_in_use
                )
                host_ports_in_use[litellm_external_port] = (
                    "LiteLLM UI (direct access, local network)"
                )
                ...
                webui_external_port = generate_random_high_port(
                    used_ports=host_ports_in_use
                )
            ...
            if use_nginx:
                ...
                litellm_internal_port = generate_random_high_port(
                    used_ports=internal_ports_in_use
                )
                postgres_port = generate_random_high_port(
                    used_ports=internal_ports_in_use
                )
                webui_internal_port = generate_random_high_port(
                    used_ports=internal_ports_in_use
                )
            ...
            if use_nginx:
                ...
                print("  • LiteLLM API: available through nginx at /api/litellm/")
                print(
                    f"  • LiteLLM UI (direct): {litellm_external_port} (local network access)"
                )
                print("  • Open WebUI: available through nginx at /")
            else:
    ...
    if use_nginx:
        ...
        print()
        print_info(
            "Note: LiteLLM UI will be available directly on port (subroute not reliably supported)"
        )
        ...
        print_info(
            "  • LiteLLM UI: needs separate port for local network access (configuration)"
        )
        print()
        litellm_external_port = select_port_interactive(
            "LiteLLM UI (external)",
            DEFAULT_LITELLM_PORT,
            "Admin UI (local network access)",
            is_internal=False,
            used_ports=host_ports_in_use,
            default_ports=default_ports,
        )
    ...
    if use_nginx:
        ...
        print(
            f"{Colors.BLUE}Internal Ports (standard, not exposed externally):{Colors.RESET}"
        )
        print()
    ...
    if use_nginx and (nginx_http_port is None or nginx_port is None):
        raise ConfigurationError(
            "Nginx port must be set when nginx is enabled. "
            "Please configure NGINX_HTTP_PORT and NGINX_PORT in your configuration."
        )
