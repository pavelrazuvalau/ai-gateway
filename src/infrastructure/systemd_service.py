...
class SystemdService:
    ...
    def is_enabled(self) -> bool:
        ...
        code, out, _ = self._run_command(
            ["systemctl", "--user", "is-enabled", self.SERVICE_NAME], check=False
        )
    ...
    def is_active(self) -> bool:
        ...
        code, out, _ = self._run_command(
            ["systemctl", "--user", "is-active", self.SERVICE_NAME], check=False
        )
    ...
    def is_lingering_enabled(self) -> bool:
        ...
        if not username:
            return False
        code, out, _ = self._run_command(
            ["loginctl", "show-user", username], check=False
        )
...
"""
    ...
    def install(self) -> bool:
        ...
        try:
            ...
            # Reload systemd
            code, _, err = self._run_command(["systemctl", "--user", "daemon-reload"])
            if code != 0:
    ...
    def enable_lingering(self) -> bool:
        ...
        try:
            ...
            code, _, err = self._run_command(["loginctl", "enable-linger", username])
            if code != 0:
    ...
    def status(self) -> Tuple[bool, str]:
        ...
        try:
            code, out, _ = self._run_command(
                ["systemctl", "--user", "status", self.SERVICE_NAME], check=False
            )
            return code == 0, out
        except Exception as e:
            return False, str(e)
    ...
    def uninstall(self) -> bool:
        ...
        try:
            ...
            # Disable service
            self._run_command(
                ["systemctl", "--user", "disable", self.SERVICE_NAME], check=False
            )
