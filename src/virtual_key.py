...
def create_virtual_key_via_api(
    ...
    try:
        ...
        print_info("Attempting to create Virtual Key via API...")
        ...
        try:
            ...
            if response.status_code in (200, 201):
                ...
                virtual_key: Optional[str] = result.get("key") or result.get("api_key")
                if virtual_key:
                ...
                if "already exists" in error_msg:
                    print_info(
                        "Virtual Key 'Open WebUI Key' already exists, trying to retrieve it..."
                    )
                    try:
                        ...
                        if list_response.status_code == 200:
                            ...
                            for key_info in keys.get("data", []):
                                if key_info.get("key_alias") == "Open WebUI Key":
                                    ...
                                    if virtual_key:
                                        print_success(
                                            f"Found existing Virtual Key: {virtual_key[:20]}..."
                                        )
                                        return virtual_key
                    except Exception:
                        pass
                print_warning(
                    f"Virtual Key creation returned status {response.status_code}"
                )
...
def check_models_visibility(litellm_url: str, master_key: str) -> Tuple[bool, int, int]:
    """
    ...
    if check_models:
        ...
        if master_key:
            ...
            if not has_models:
                ...
                print_info(
                    "      • Note: Public models still require API key (Virtual Key)"
                )
                print_info(
                    "      • 'Public' only means 'visible to all users', not 'no auth required'"
                )
                print()
                ...
                print_warning(
                    f"⚠️  Only {public_count}/{total_models} models are public"
                )
                print()
                ...
                print_info(
                    "   5. Note: Public models still require API key (Virtual Key)"
                )
                print()
            else:
...
def setup_virtual_key_interactive(non_interactive: Optional[bool] = None) -> Optional[str]:
    """
    Interactive setup for Virtual Key creation and Open WebUI configuration.

    Args:
        non_interactive: If True, skip prompts and try to create Virtual Key automatically. If None, check NON_INTERACTIVE env var.
    ...
    if non_interactive:
        print_info("Non-interactive mode: attempting automatic Virtual Key creation")
    else:
    ...
    # Load configuration
    project_root = Path(".")
    file_repo = FileRepository(project_root)
    env_vars = file_repo.read_env_file(Path(".env"))
    ...
    if not virtual_key:
        ...
        if use_nginx:
            print_info(
                "Note: Open WebUI will use Docker network URL (http://litellm:4000) internally"
            )
    ...
    if not virtual_key:
        ...
        print()
        if non_interactive:
            # In non-interactive mode, cannot proceed without Virtual Key
            print_error("❌ Virtual Key is required but was not created automatically")
            print_error("System cannot start without Virtual Key")
            print_info("Please run virtual-key.sh manually or set VIRTUAL_KEY in .env")
            return None
    ...
    if virtual_key:
        print()
        configure_openwebui_connection(
            virtual_key, litellm_url, use_nginx=use_nginx, check_models=True
        )
        print()
        print_success("✅ Virtual Key setup complete!")
...
def main() -> int:
    ...
    try:
        ...
        env_vars = file_repo.read_env_file(Path(".env"))
        if (
            env_vars.get("FIRST_RUN", "no").lower() in ("yes", "true", "1")
            or virtual_key
        ):
                ...
                if virtual_key:
                    ...
                    for _i, line in enumerate(lines):
                        if skip_next:
                            skip_next = False
                            continue
                        if line.startswith("VIRTUAL_KEY=") or line.strip().startswith(
                            "# Virtual Key for Open WebUI"
                        ):
                            continue
                    ...
                    for i, line in enumerate(new_lines):
                        if line.startswith("FIRST_RUN="):
                            ...
                            new_lines.insert(
                                i + 2,
                                "# Virtual Key for Open WebUI (created via virtual-key.sh/virtual-key.bat)",
                            )
