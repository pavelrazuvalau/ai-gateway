...
│   ├── script_init.sh      # Bash script initialization (shared module)
│   ├── script_init.bat      # Windows batch script initialization (shared module)
...
- **`src/script_init.sh`**: Common bash initialization
- **`src/script_init.bat`**: Common Windows batch initialization
  - Color definitions
...
3. **Dependency check**:
   ...
   - Fallback: Bash `run_standard_checks()` from `script_init.sh`
...
- **Shared bash module**: `script_init.sh` for common functions
- **Shared Windows module**: `script_init.bat` for common functions
