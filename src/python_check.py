...
from .platform_utils import PlatformType, check_python_installed, detect_platform
...
def check_python() -> Tuple[bool, Optional[str]]:
    ...
    if not is_installed:
        ...
        if current_platform == PlatformType.WINDOWS:
            ...
            try:
                open_browser = (
                    input("Open Python download page in browser? [Y/n]: ")
                    .strip()
                    .lower()
                )
                if open_browser != "n":
            ...
            try:
                ...
                print()
                install_brew = (
                    input("Install Python via Homebrew now? [y/N]: ").strip().lower()
                )
                if install_brew == "y":
                    try:
                ...
                print()
                print(
                    "Or download Python manually: https://www.python.org/downloads/macos/"
                )
