...
**Applicability**: [All models | GPT-4+ | Claude | Specific cases]
...
**System Prompt:** A prompt that defines the role, behavior, and operating rules of an AI agent. Usually passed through the system message in the API. **Important:** A system prompt is not a program, but a set of instructions for guiding the model toward correct decision-making through many intermediate steps. It provides instructions on how to gather context and how to use decision-making principles to achieve the desired result. See [Nature of System Prompt](#nature-of-system-prompt).
...
**Important:** In some development environments, when a tool call fails (e.g., `write` returns an error), the entire chat session terminates and the agent stops working. This means error handling after the fact does NOT work. Need to focus on error prevention and alternative strategies BEFORE creating the file.
...
**Related sections:** [Best Practices](#best-practices), [Working with Tools and File Creation](#working-with-tools-and-file-creation)
...
Modern LLMs (GPT-4, Claude 3, etc.) have extensive training and sophisticated understanding. Providing excessive examples can:
