...
from ..core.constants import (
    DOCKER_TIMEOUT, DOCKER_COMPOSE_TIMEOUT, DOCKER_UP_TIMEOUT, DOCKER_DOWN_TIMEOUT
)
...
class DockerClient:
    ...
    def get_running_containers(filter_name: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            if hasattr(e, 'stderr') and e.stderr:
                logger.debug(f"Docker error output: {e.stderr}")
            return []
    ...
    def compose_up(work_dir: str, detach: bool = True, wait: bool = False) -> None:
        ...
        try:
            ...
            # If --wait failed, error was already shown to user (output not captured)
            # For non-wait mode, stderr might be available
            if not wait and e.stderr:
                logger.debug(f"Docker compose error output: {e.stderr}")
            raise DockerError(f"Cannot start containers: {e}") from e
        except subprocess.TimeoutExpired as e:
            logger.error(f"Timeout starting containers: {e}")
            raise DockerError(f"Cannot start containers: timeout") from e
    ...
    def wait_for_containers(work_dir: str, timeout: int = 300) -> bool:
        ...
        logger.warning(f"Timeout waiting for containers to become healthy (waited {timeout}s)")
        return False
