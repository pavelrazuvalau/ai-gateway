...
from ..utils import (
    ...
    Colors, read_env_file, _ask_yes_no
)
...
class ContinueDevService:
    ...
    def __init__(self, project_root: Path):
        ...
        self.output_config_file = self.project_root / "continue-dev-config-generated.yaml"
    ...
    def get_api_config_from_env(self) -> Tuple[Optional[str], Optional[str], bool]:
        ...
        """
        env_vars = read_env_file(self.project_root / ".env")
        ...
        if use_nginx:
            nginx_port = env_vars.get("NGINX_HTTP_PORT", "").strip() or env_vars.get("NGINX_PORT", "").strip()
            if nginx_port:
    ...
    def generate_model_name(self, model_id: str) -> str:
        ...
        # Claude models: "claude-haiku-4-5" -> "Claude Haiku 4.5"
            ...
            result = re.sub(r'(^|-)([a-z])', lambda m: m.group(1) + m.group(2).upper(), model_id)
    ...
    def model_supports_tool_use(self, model_id: str) -> bool:
        ...
        return (
            ...
            ('gemini' in model_lower and 'gemini-pro-vision' not in model_lower)
        )
            ...
            if context_length:
                ...
                if provider == 'anthropic':
                    if 'haiku' in model_id.lower():
                        tier_limits = {1: "50k", 2: "1M", 3: "2M", 4: "4M"}.get(tier, "50k")
                    else:
                        tier_limits = {1: "30k", 2: "500k", 3: "1M", 4: "2M"}.get(tier, "30k")
                    tier_note += f" ITPM limit"
...
"""
    ...
    def generate_system_prompt(
        ...
        stop_rules = """1. Errors found → STOP → Report → Wait (DO NOT fix automatically)
2. After completing one task → STOP → Update plan → Provide summary → Ask "Ready?"
3. Large context (>15k Sonnet/Opus, >30k Haiku) → STOP → Summary → Wait""" if tier == 1 else """1. Complex errors or unclear fixes → STOP → Report → Wait
   - Fix simple errors (syntax, typos) automatically if clear"""
        
        file_instruction = "- Read only files directly related to current task (minimize context usage)" if tier == 1 else ""
        
        agents_instruction = "- Read @AGENTS.md (workspace root) in FIRST request using read_file, store in session context" if has_agents_md else ""
        
        footer = "**ONE TASK PER REQUEST. ALWAYS STOP AFTER TASK.**" if tier == 1 else "**Work efficiently. Stop on errors or when user requests.**"
...
"""
    ...
    def run_setup_interactive(self) -> int:
        """
        ...
        print_info("Examples:")
        print(f"  {Colors.GREEN}• http://localhost:63345/api/litellm/v1{Colors.RESET} (with Nginx, local)")
        print(f"  {Colors.GREEN}• http://localhost:4000/v1{Colors.RESET} (direct LiteLLM, local)")
        print()
        ...
        print_warning("   Models are created through LiteLLM Admin UI and are only accessible via Virtual Key")
        print_warning("   Master Key cannot access models created through UI")
        print()
        
        if virtual_key:
            masked = self.mask_api_key(virtual_key)
            print_info(f"Found Virtual Key in .env (press Enter to use, or enter different key):")
        ...
        print_warning("Note: Tier selection applies ONLY to Anthropic models (contextLength configuration)")
        print()
        print(f"{Colors.GREEN}[1] Tier 1{Colors.RESET} - Basic limits (50 RPM, 30-50k ITPM, 8-10k OTPM)")
        print(f"{Colors.GREEN}[2] Tier 2{Colors.RESET} - Standard limits (1k RPM, 500k-1M ITPM, 50-100k OTPM)")
        print(f"{Colors.GREEN}[3] Tier 3{Colors.RESET} - Higher limits (2k RPM, 1-2M ITPM, 100-200k OTPM)")
        print(f"{Colors.GREEN}[4] Tier 4{Colors.RESET} - Maximum limits (4k RPM, 2-4M ITPM, 200-400k OTPM)")
        print()
        
        tier_input = input("Select tier [1-4] (default: 1): ").strip()
        tier = int(tier_input) if tier_input and tier_input.isdigit() and 1 <= int(tier_input) <= 4 else 1
        ...
        if has_agents_md:
            ...
            print()
            has_agents_md = _ask_yes_no("Do you have AGENTS.md file in your repository?", default=False)
        ...
        config_content = self.generate_config_yaml(models, api_base, virtual_key, tier, has_agents_md)
        ...
        print(f"   Config: {Colors.GREEN}continue-dev-config-generated.yaml{Colors.RESET}")
        print(f"   System prompt: {Colors.GREEN}.continue/prompts/system-prompt.md{Colors.RESET}")
        print()
        
        print_info("📝 Setup steps (on client machine with VS Code):")
        print()
        print_info("1. Copy config file:")
        print(f"   {Colors.GREEN}cp continue-dev-config-generated.yaml <continue-dev-config-dir>/config.yaml{Colors.RESET}")
        print()
        print_info("2. Enable system prompt in config.yaml:")
        print(f"   Edit config.yaml and uncomment:")
        print(f"   {Colors.GREEN}prompts:{Colors.RESET}")
        print(f"   {Colors.GREEN}  - uses: .continue/prompts/system-prompt.md{Colors.RESET}")
        print()
