...
def generate_env_file(
    ...
    """
    from .core.constants import DEFAULT_UI_USERNAME, MAX_PORT, MIN_PORT
    ...
    if not (MIN_PORT <= postgres_port <= MAX_PORT):
        raise ValidationError(
            f"PostgreSQL port must be between {MIN_PORT} and {MAX_PORT}"
        )
    ...
    # Internal ports
    env_content.append(
        "# Internal ports (only for container-to-container communication)"
    )
    ...
    if port_config.get("use_nginx"):
        ...
        env_content.append("# LiteLLM API available through nginx at /api/litellm/")
        env_content.append(
            "# LiteLLM UI has separate external port for local network access"
        )
        ...
        env_content.append(
            f"LITELLM_EXTERNAL_PORT={port_config.get('litellm_external_port', '')}"
        )
        env_content.append(
            f"WEBUI_EXTERNAL_PORT={port_config.get('webui_external_port', '')}"
        )
    env_content.append("")
    ...
    nginx_http_port = port_config.get("nginx_http_port") or port_config.get(
        "nginx_port", ""
    )
    ...
    if preserve_first_run:
        ...
        if env_file.exists():
            try:
                ...
                existing_env = file_repo.read_env_file(env_file)
        ...
        if existing_virtual_key:
            env_content.append("")
            env_content.append(
                "# Virtual Key for Open WebUI (preserved from existing .env)"
            )
        ...
        env_content.append("")
        env_content.append(
            "# Virtual Key for Open WebUI (REQUIRED - created via virtual-key.sh/virtual-key.bat)"
        )
